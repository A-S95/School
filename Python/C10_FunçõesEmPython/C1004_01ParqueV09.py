...
# Função "validaLugar()"
###############################################
def validaLugar(lugares, numLugar, estado, cadeia):
    if not numLugar.isdigit():
        ...
        if numLugar < 1 or numLugar > len(lugares):
            ...
            if lugares[numLugar - 1] == estado:
                print("O lugar já está "+cadeia+"!")
                return False
            else:
                return True
...
def marcarLugar(lugares, estadoOCUPADO):
    if (obtemNumLugaresOcupados(lugares, estadoOCUPADO)
        ...
        print("Qual o lugar a ocupar (1-"
              + str(len(lugares)) + ")? ", end="")
        numLugar = input()
        if validaLugar(lugares, numLugar, estadoOCUPADO,
                       "ocupado"):
            numLugar = int(numLugar)
...
def libertarLugar(lugares, estadoLIVRE):
    print("Qual o lugar a libertar (1-"
          + str(len(lugares)) + ")? ", end="")
    numLugar = input()
    if validaLugar(lugares, numLugar, estadoLIVRE,
                   "livre"):
        numLugar = int(numLugar)
